    ...
    public void onResponsePotionThrow(ExtendedEventArgs eventArgs)
    {
        ...
        if (Constants.USER_ID != args.user_id)
        {
            ...
            if (args.user_id == 1)
            {
                ...
                if (args.z == 1)
                {
                    if(players[currentPlayer].holster.cardList[args.y - 1].card.cardType == "Potion")
                    {
                        Debug.Log("Change this client");
                        td.addCard(players[currentPlayer].holster.cardList[args.y - 1]);
                        players[0].subHealth(args.w);
                    } else if (players[currentPlayer].holster.cardList[args.y - 1].card.cardType == "Vessel")
                    {
                        // TODO

                    } else if (players[currentPlayer].holster.cardList[args.y - 1].card.cardType == "Artifact")
                    {
                        // TODO
                    }
                }
